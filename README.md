...
- **`fill()` can silently desync from React's state, even with no attach involved.** Playwright's `fill()` sometimes sets the contenteditable's DOM content without firing the synthetic input event React's own state tracking listens for — the box visually shows the text, but React still thinks it's empty, so `Enter` submits nothing. Observed intermittently in CDP-attach mode against a real Chrome. `_fill_prompt` in `browser.py` checks whether the fill actually registered (re-reads the box's text) and, if not, falls back to `document.execCommand('insertText', ...)`, which fires a real `InputEvent` React's synthetic system does pick up.
