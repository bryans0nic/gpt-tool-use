...
- **`fill()` can silently desync from React's state, even with no attach involved.** Playwright's `fill()` sometimes sets the contenteditable's DOM content without firing the synthetic input event React's own state tracking listens for — the box visually shows the text, but React still thinks it's empty, so `Enter` submits nothing. Observed intermittently in CDP-attach mode against a real Chrome.
- **`fill()` is also outright unreliable on large prompts** — a detailed bug report traced `Locator.fill: Timeout 5000ms exceeded` failures to prompts in the ~8-12k character range. ChatGPT's prompt box is a ProseMirror contenteditable, not a plain textarea; `fill()`'s internal actionability/select-all handling can lose the locator mid-fill as the editor re-renders on a large payload. `_fill_prompt` in `browser.py` now uses `keyboard.insert_text()` instead — one atomic CDP `Input.insertText` call, no per-character events, no intermediate re-render window to go stale in, and (as a side effect) it reliably fires the input events React needs, which also covers the desync case above without needing the `execCommand` fallback in the common case. Verified live with a 9,042-char prompt at `effort="high"` (clean 17.4s round trip) — that specific length previously failed.
- **The upload-spinner selector had a permanent false positive.** `attach_file`'s "wait for upload to finish" loop counted any element matching `[class*="animate-spin"]`, but the page has an unrelated **hidden** `<svg>` icon elsewhere that always matches — confirmed live by polling the selector every second and logging each match's visibility. Since `count()` doesn't check visibility, every attach silently burned its *entire* wait budget (60s + a 15s recovery attempt) regardless of how fast the real upload finished, which surfaced as a phantom timeout even when the send+response had already succeeded underneath. Fixed by only counting *visible* matches — attach time dropped from that wasted 60-75s to ~1.7s in testing.
- **Multiple attachments keep the send button `disabled` until ChatGPT finishes processing them server-side** — a real, legitimate state, not a bug. The old fallback clicked the button immediately; Playwright's own actionability wait for "enabled" then silently ate its 30s default timeout before failing. This was the actual root cause behind `attach_files` with multiple files never landing at all (as opposed to the single-file case above, which did land but reported a phantom timeout). `_send_prompt` now polls `is_disabled()` for up to ~20s before clicking, only falling back to re-fill-and-retry if the button never becomes enabled in that window. Verified live: a 2-file `attach_files` call that previously never completed finished cleanly in 31s, correctly identifying both files.
- **`attach_file` has no reliable positive "it attached" signal to poll for — confirmed live there's nothing consistent to check.** Small/fast files never show a visible spinner *or* any filename text at all (tight-polled every 100ms from the moment of drop — zero matches, the whole time); the drop-preview overlay's filename chip closes as soon as the drag state ends. So completion detection stays negative-signal-only (spinner gone), which can't distinguish "attached instantly" from "silently did nothing." Hardened around that gap instead of chasing a selector that doesn't reliably exist: (1) pre-flight checks reject a missing/unreadable/empty/oversized file before ever touching the browser, with a clear Python exception instead of a cryptic downstream failure; (2) a phrase-matched rejection check (same approach as the existing rate-limit detector — "couldn't be uploaded", "file is too large", "unsupported file type", etc.) raises immediately if ChatGPT visibly rejects the file, instead of silently waiting out the full timeout budget for something that already failed; (3) a small fixed settle margin (400ms) after the spinner-gone signal, since — same lesson as the disabled-send-button fix — ChatGPT's internal "attached" state can lag a beat behind what's visible.
- **How completion is detected, and why the old "wait for message to appear" ceiling was unsafe for high reasoning effort.** Confirmed live (`browser.py`'s `_is_generating`): the stop button (`[data-testid="stop-button"]` / `[aria-label="Stop generating"]`) appears within ~3s of sending — well before any `data-message-author-role="assistant"` DOM node exists — and stays visible through the entire "thinking" phase, which routinely runs past 30s on `effort="pro"`/`"extra high"`. The old 30s hard ceiling on "wait for an assistant message to appear" didn't check it, so a long thinking phase looked identical to a genuinely stuck send and triggered an unnecessary resend — a real duplicate-message risk. That wait now only counts against its budget while the stop button is *not* visible; once streaming starts, completion is still detected the existing way (response text stable for 5 consecutive polls, `result-streaming` class gone, and the stop button gone too).
- **Stuck-state recovery is deterministic, not AI-driven — by design, for now.** When a send or file upload times out, `_attempt_recovery` in `browser.py` tries one narrow, safe fallback (press Escape, click a known dismiss button like "Got it"/"Close") and retries once before raising. This was chosen over a vision-model-in-the-loop fallback (screenshot → ask an LLM what to click → execute it) because that would need an `ANTHROPIC_API_KEY`, incur per-call cost, and — the bigger issue — an LLM autonomously clicking around a real authenticated ChatGPT account has real failure modes (wrong send, wrong project, unintended navigation) that a fixed dismiss-only action set can't have. If the deterministic recovery proves insufficient for stuck states it can't name, that's the signal to revisit this, with the action space tightly constrained rather than left open-ended.
- **A cross-tab "remembered draft" race can silently clobber a fill-in-progress — now explicitly detected.** ChatGPT appears to restore a remembered draft into a fresh "new chat" composer from shared (localStorage/cross-tab) state, and this can race with and overwrite content this tool just filled — observed live: once as literal stale draft text left over from an unrelated orphaned session, once mid-test where a large-prompt + `attach_files` call silently had its filled content replaced by that same stale draft (confirmed via a screenshot showing unrelated content sitting in the box after 3 retries, 108s instead of the normal ~17s — it self-healed only because the retry loop happened to be generous enough, not because anything recognized what had happened). `_send_prompt` in `browser.py` now explicitly checks for this: after `Enter`, if the box holds non-empty content that doesn't match what was sent, that's treated as its own failure mode (clobbered, not sent and not still-pending) — screenshots to `debug_send_clobbered.png`, clears the box, and retries immediately, rather than folding it into the generic "still there" path where it would previously have been misread as already sent. Verified live with a forced deterministic repro (a patched `inner_text()` returning corrupted content on exactly the post-Enter check) — the clobber branch fired, screenshot was written, and the retry succeeded cleanly.
