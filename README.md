...
- **Known, unresolved: a cross-tab "remembered draft" race can silently clobber a fill-in-progress, self-healing but slowly.** ChatGPT appears to restore a remembered draft into a fresh "new chat" composer from shared (localStorage/cross-tab) state, and this can race with and overwrite content this tool just filled — observed live twice: once as literal stale draft text left over from an unrelated orphaned session, once mid-test where a large-prompt + `attach_files` call silently had its filled content replaced by that same stale draft, confirmed via a `debug_send_prompt_failed.png` screenshot showing unrelated content sitting in the box. The existing retry logic in `_send_prompt`/`stream_message` treats "box content differs from what we sent" the same as "box is empty, must be sent" in some paths, so it doesn't immediately recognize this as corruption — the call in question **did** eventually succeed (3 retries, 108s instead of the normal ~17s), but only because the retry loop is generous, not because the underlying race is understood or fixed. Not chased further yet — the fix likely needs to explicitly distinguish "content differs from what we intended" as its own failure mode from "content is gone because it sent," rather than treating anything non-matching as success.
